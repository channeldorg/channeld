...
func TestSphereAOI(t *testing.T) {
	...
	query2.SphereAOI.Center.X = 5
	query2.SphereAOI.Center.Z = 5
	result, err = ctl2.QueryChannelIds(query2)
	assert.NoError(t, err)
	assert.Len(t, result, 1)
	assert.Contains(t, result, common.ChannelId(65539))

	query2.SphereAOI.Radius = 5
	result, err = ctl2.QueryChannelIds(query2)
	assert.NoError(t, err)
	...
	result, err = ctl2.QueryChannelIds(query2)
	assert.NoError(t, err)
	// Should contain 65539, 65537, 65538
...
func TestBoxAOI(t *testing.T) {
	...
	query2.BoxAOI.Center.X = 5
	query2.BoxAOI.Center.Z = 5
	result, err = ctl2.QueryChannelIds(query2)
	assert.NoError(t, err)
	assert.Len(t, result, 1)
	assert.Contains(t, result, common.ChannelId(65539))

	query2.BoxAOI.Extent.X = 5
	query2.BoxAOI.Extent.Z = 5
	result, err = ctl2.QueryChannelIds(query2)
	assert.NoError(t, err)
	// Should only contain 65539
	assert.Len(t, result, 1)
